...
$no=0;
$th=date("Y");
$ambildata=mysql_query("SELECT YEAR(bulan) AS tahun, sum(jumlah) AS npaw FROM rekap_keuangan where konfirmasi='ter-uproav' GROUP BY YEAR(bulan)");
...
$ambildata2=mysql_query("SELECT SUM(jumlah) as npe FROM rekap_keuangan GROUP BY YEAR(bulan) ");
