...
$kd =$_GET["kdunix"];
$kode_username =mysql_fetch_array(mysql_query("select * from user_admin where kode='$kd'"));
echo " <center>
...
<tr><td>Hak Akses</td><td>
    <select name='hak_akses'>
         <option>".$kode_username[hak_akses]."</option>
...
<tr><td>Wilayah Admin</td><td>
    <select name='wilwil'>
         <option>".$kode_username[wilayah]."</option>
         <option></option>
...
<tr><td>Username</td><td><input type='text' name='username' class='form-control' value='".$kode_username[username]."' readonly></td></tr>
<tr><td>Password</td><td><input type='text' name='password' class='form-control' value='".$kode_username[password]."'></td></tr>
<tr><td>Nama</td><td><input type='text' name='nama' class='form-control' value='".$kode_username[nama]."'></td></tr>
<tr><td>Jabatan</td><td><input type='text' name='jabatan' class='form-control' value='".$kode_username[jabatan]."'></td></tr>
<tr><td>No Hp</td><td><input type='text' name='no_hp' class='form-control' value='".$kode_username[no_hp]."'></td></tr>
<tr><td>Email</td><td><input type='text' name='email' class='form-control' value='".$kode_username[email]."'></td></tr>
...
if($tipe_file == "image/jpeg" || $tipe_file == "image/png"){ 
    if($ukuran_file <= 1000000){
        if(move_uploaded_file($tmp_file, $path)){ 
            ...
            $query = "update user_admin set hak_akses='$_POST[hak_akses]',wilayah='$_POST[wilwil]',nama='$_POST[nama]',foto_profil='".$nama_file."',password='$_POST[password]',jabatan='$_POST[jabatan]',no_hp='$_POST[no_hp]',email='$_POST[email]',publish='publish' where kode='$kd'";
             $sql = mysql_query($query); 
            if($sql){ 
			 echo"<script>alert('Data Tersimpan');window.location='edit_admin.php?kdunix=".$kd."'</script>";
            }else{
...
{
            $query = "update user_admin set hak_akses='$_POST[hak_akses]',wilayah='$_POST[wilwil]',nama='$_POST[nama]',password='$_POST[password]',jabatan='$_POST[jabatan]',no_hp='$_POST[no_hp]',email='$_POST[email]',publish='publish' where kode='$kd'";
          $sql = mysql_query($query); 
            if($sql){ 
