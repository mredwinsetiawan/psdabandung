...
include"../koneksi.php";
$prom=mysql_query("SELECT * from wilayah");
...
$no=0;
$th=date("Y");
$ambildata=mysql_query("SELECT YEAR(bulan) AS tahun,month(bulan) AS bulan, sum(npa) AS npaw ,sum(volume) as vol FROM rekap_admin_input where korwil='".$promise[wilayah]."' and year(bulan)='$thn1' GROUP BY month(bulan)");
...
$no=0;
$th=date("Y");
$ambildata=mysql_query("SELECT YEAR(bulan) AS tahun,month(bulan) AS bulan, sum(npa) AS npaw ,sum(volume) as vol FROM rekap_admin_input where korwil='".$promise[wilayah]."' GROUP BY MONTH(bulan)");
$cekdata=mysql_num_rows($ambildata);
if($cekdata=='0'){
echo "<font color='red'><center>Maaf Data tidak ada</center></font>";
}
