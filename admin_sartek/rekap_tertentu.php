...
include"../koneksi.php";
$no=0;
$ambildata=mysql_query("SELECT wilayah,YEAR(bulan) AS tahun,month(bulan) AS bulan,sum(jumlah) as jumlah FROM rekap_sartek where year(bulan)='$thn1' GROUP BY wilayah");
...
<?
$th=date("Y");
$ambildata3=mysql_query("SELECT SUM(jumlah) as jumlahiah FROM rekap_sartek where YEAR(bulan)='$thn1' group by YEAR(bulan)='$thn1' ");
...
$no=0;
$th=date("Y");
$ambildata=mysql_query("SELECT wilayah as wilayah,YEAR(bulan) AS tahun,month(bulan) AS bulan,sum(jumlah) AS jumlahh FROM rekap_sartek where year(bulan)='$th' GROUP BY wilayah");
$cekdata=mysql_num_rows($ambildata);
if($cekdata=='0'){
echo "<font color='red'><center>Maaf Data tidak ada</center></font>";
}
...
<?
$th=date("Y");
$ambildata2=mysql_query("SELECT SUM(jumlah) as jumlahan FROM rekap_sartek where year(bulan)='$th' group by year(bulan)='$th'");
